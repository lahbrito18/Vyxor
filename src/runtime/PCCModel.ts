...
const FileNameMap: Record<string, string | undefined> = {
  ...
  '120mm_fan_sample': 'Ventoinha de 120 mm',
  atx_motherboard_sample: 'Placa-mãe ATX',
  cooler_sample: 'Cooler a ar',
  cpu_sample: 'Processador',
  gpu_sample: 'Placa de vídeo',
  ddr4_ram_sample: 'Memória RAM DDR4',
  nvme_ssd_sample: 'SSD NVMe',
  atx_power_sample: 'Fonte ATX',
};

/**
 * Descreve um modelo de componente de PC
 */
export class PCCModel {
  public readonly taskExecutor: TaskExecutor;

  /**
   * Nó de transformação raiz (Root transform node)
   */
  public readonly root: PCCRootTransformNode;

  /**
   * URL de origem do modelo
   */
  ...
  /**
   * Tipo do produto (ex: "case", "motherboard", "cpu", "gpu", "ram", "storage")
   */
  public readonly type: ComponentType;

  /**
   * Lista de alvos de alternância (toggle targets)
   */
  public readonly toggleTargets: ToggleTarget[];

  /**
   * Lista de pontos de montagem (mount points)
   */
  ...
  public createInstance(name: string): PCCModel {
    // todo: clone perfeito quando a animação estiver rodando
    ...
    while (stack.length > 0) {
      ...
      if (visited.has(original)) {
        console.warn('Referência circular detectada');
        continue;
      }
      ...
      for (let i = 0; i < descendants.length; ++i) {
        ...
        if (
          // pula o modelo anexado (attached model)
    ...
    // atualiza referências de metadata
    for (const nodeInfo of nodeInfoMap.values()) {
      ...
      // coleta toggle targets, mount points e animações
